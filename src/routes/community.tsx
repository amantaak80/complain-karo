...
import { ArrowLeft, ThumbsUp, MessageCircle, Share2, BadgeCheck, Clock, CheckCircle2, AlertCircle } from "lucide-react";

export const Route = createFileRoute("/community")({
  head: () => ({
    meta: [
      { title: "Voice Feed — ComplainKaro" },
      { name: "description", content: "A community feed of complaints and how local leaders are responding. Like, comment, and amplify." },
    ],
  }),
...
const posts = [
  {
    id: 1, author: "Ramesh Patil", role: "Sarpanch · Wai", verified: true,
    when: "2h", status: "in-progress" as const,
    ...
    likes: 124, comments: 18, level: "Local",
  },
  {
    id: 2, author: "Priya Sharma", role: "Resident", verified: false,
    when: "5h", status: "open" as const,
    ...
    likes: 89, comments: 12, level: "Local",
  },
  {
    id: 3, author: "DM Office, Pune", role: "District Collector", verified: true,
    when: "1d", status: "resolved" as const,
    ...
    likes: 502, comments: 73, level: "State",
  },
  {
    id: 4, author: "Anil Kumar", role: "Resident", verified: false,
    when: "2d", status: "open" as const,
    title: "Pothole on NH-48 service road",
    body: "Two bike accidents this week. NHAI has been tagged.",
    likes: 211, comments: 41, level: "Country",
  },
];

const statusMap = {
  open: { label: "Open", icon: AlertCircle, cls: "bg-destructive/15 text-destructive" },
  "in-progress": { label: "In progress", icon: Clock, cls: "bg-primary/15 text-primary" },
  resolved: { label: "Resolved", icon: CheckCircle2, cls: "bg-accent/15 text-accent" },
};
    ...
    <div className="px-4 pt-6">
      <div className="mb-4 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Community complaints & responses</p>
        </div>
      </div>

      <div className="mb-3 flex gap-2 overflow-x-auto pb-1">
        {["For You", "Nearby", "Local", "State", "Country", "Resolved"].map((f, i) => (
          <button
            key={f}
            className={`whitespace-nowrap rounded-full border px-3 py-1.5 text-xs font-semibold ${
              i === 0 ? "border-secondary bg-secondary text-secondary-foreground" : "border-border bg-card text-muted-foreground"
            }`}
          >
            {f}
          </button>
            ...
            <article key={p.id} className="overflow-hidden rounded-3xl border border-border bg-card">
              <header className="flex items-center gap-3 p-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[11px] text-muted-foreground">{p.role} · {p.when}</p>
                </div>
              ...
              <div className="px-4 pb-3">
                ...
                <div className={`mt-3 inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-semibold ${S.cls}`}>
